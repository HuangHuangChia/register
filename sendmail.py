...
def _sendmail_inlist(send_user = 'WebPage Test<Doraemon@typc.mohw.gov.tw>' ,receive_users = '<honga@typc.mohw.gov.tw>',
...
#server_address = 'msa.hinet.net'   #伺服器地址
    ...
    #  檢查檔案是否存在
        if os.path.isfile(fi):  
            part_attach1 = MIMEApplication(open(fi,'rb').read())   #開啟附件
            part_attach1.add_header('Content-Disposition','attachment',filename=fi) #為附件命名
